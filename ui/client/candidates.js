/**  Get new  parties
   ...
   */
    ...
    let getMyOfiice = document.getElementById('hiddencandidateOffice');
    let selectHome =`<select class="candidate-selection" name="office" id="selectBox" >
                    <option>Name</option>`;
                    let selectEnd=`</select>`;

                    let mycandidate = document.getElementById('hiddenofficeTwo');
                    let selectCandidateHome =`<select class="candidate-selection" name="party" id="selectBox" >
  ...
  const getOffice = (e) => {
    ...
    const notify = document.getElementById('notifyCandidates');
    ...
    fetch('https://dry-journey-38911.herokuapp.com/api/v1/offices', {
      ...
      .then(res => res.json()
        .then((data) => {
          ...
          document.getElementById('hiddencandidateOffice').innerHTML = selectHome + getMyOfiice + selectEnd;
        })
    ...
    fetch('https://dry-journey-38911.herokuapp.com/api/v1/offices', {
        ...
        .then(res => res.json()
          .then((data) => {
            ...
            document.getElementById('hiddenofficeTwo').innerHTML = selectCandidateHome + mycandidate + selectCandidateEnd;
          })
          .catch(err => console.error(err)));
  };
...
const mycandidateForm = document.getElementById("candidateForm");

const login = e => {
  e.preventDefault(e);
  const getFormName = document.forms.candidateForm;
  const party = getFormName.party.value;
...
mycandidateForm.addEventListener("submit", login, false);
